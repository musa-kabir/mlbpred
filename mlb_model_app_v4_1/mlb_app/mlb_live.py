...
_last_logged = {}   # v4.1 dedupe cache
...
def snapshot_live_games():
    ...
    for g in statsapi.schedule(date=date.today().strftime("%Y-%m-%d")):
        ...
        rows.append(row)
        # v4.1: only log when the game state actually changed
        sig = (row["game_pk"], row["score"], row["inning"], row["outs"], row["bases"])
        if sig != _last_logged.get(row["game_pk"]):
            with open(LIVE_LOG, "a") as f:
                f.write(json.dumps(row) + "\n")
            _last_logged[row["game_pk"]] = sig
    return rows
